...
function loadKeyCharacter(namer) {
  const keyCharacter = $("input[name='key-character']").val();
  log(keyCharacter);
  namer.loadKeyCharacter(keyCharacter);
}

function loadNameAmount() {
  const amount = $("input[name='result-size']").val();
  return amount ? amount : nameAmount;
}
...
function initEvents(namer) {
  ...
  sel('.btn-go').addEventListener('click', () => {
    setLoading();
    const n = loadNameAmount();
    const html = [];
    loadKeyCharacter(namer)
    for (let i = 0; i < n; i++) {
